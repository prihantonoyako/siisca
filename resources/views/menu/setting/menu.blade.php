				...
				<div class="row">
					<div class="table-responsive">
						<table class="table table-bordered" id="tabel_menu" width="100%" cellspacing="0">
							<thead>
								<tr>
									<th colspan="6" class="text-center">Informasi Menu</th>
							...
							<tbody>
								...
								<tr>
									...
									<td>
										@if($item->is_aktif==1)
										<input id="{{$item->id_menu}}" class="toggle-event" type="checkbox" checked data-toggle="toggle" data-on="Active" data-off="Inactive" data-onstyle="success" data-offstyle="danger">
										@else
										<input id="{{$item->id_menu}}" class="toggle-event" type="checkbox" data-toggle="toggle" data-on="Active" data-off="Inactive" data-onstyle="success" data-offstyle="danger">
										@endif
									</td>
									<td><a href="{{url('/setting/menu')}}/{{$item->id_menu}}/edit" class="btn btn-primary"><i class="fas fa-edit"></i></a></td>
									<td>
										<a href="#" class="btn btn-danger btn-circle btn-sm deleteRecord" data-toggle="modal" data-target="#deleteModal" data-id="{{ $item->id_menu }}">
...
<script>
	$(document).ready(function() {
		...
		$(document).on("change", ".toggle-event", function() {
			...
			var url = "{{url('setting/menu/')}}/" + id_menu;
			$.ajax({
				url: url,
				type: "PATCH",
				cache: false,
